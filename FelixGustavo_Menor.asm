...
output_msg: .asciiz "El numero menor es: "  # Mensaje para mostrar el número menor
...
min_value: .word 0  # Variable para almacenar el número menor
...
read_numbers:
    beqz $t4, find_min  # Si $t4 es 0, saltar a find_min
    li $v0, 4  # Código de syscall para imprimir cadena
...
find_min:
    # Inicializar el valor mínimo con el primer número
    la $t3, numbers  # Cargar la dirección base del array numbers en $t3
    lw $t5, 0($t3)  # Cargar el primer número en $t5
    sw $t5, min_value  # Guardar el primer número como el valor mínimo inicial
...
compare_numbers:
    beqz $t4, print_min  # Si $t4 es 0, saltar a print_min
    lw $t6, 0($t3)  # Cargar el siguiente número en $t6
    lw $t5, min_value  # Cargar el valor mínimo actual en $t5
    bge $t6, $t5, next_number  # Si $t6 es mayor o igual a $t5, saltar a next_number
    sw $t6, min_value  # Si $t6 es menor, actualizar el valor mínimo
next_number:
    addi $t3, $t3, 4  # Incrementar la dirección base del array en 4 bytes
...
print_min:
    ...
    lw $a0, min_value  # Cargar el valor mínimo en $a0
